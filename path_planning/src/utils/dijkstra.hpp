...
namespace dijkstra {
bool dijkstra(const std::vector<std::vector<bool>>& grid_map,
  ...
  parent[start(0)][start(1)] = Eigen::Vector2i(-1, -1);
  ...
  while (!open.empty()) {
    ...
    for (auto& d : dir) {
      ...
      if (g[next(0)][next(1)] < 0x3f3f3f3f) {
        if (g[next(0)][next(1)] > g[current(0)][current(1)] + d.lpNorm<2>()) {
          g[next(0)][next(1)] = g[current(0)][current(1)] + d.lpNorm<2>();
          f[next(0)][next(1)] = g[next(0)][next(1)] + (goal - next).lpNorm<1>();
        ...
        f[next(0)][next(1)] = g[next(0)][next(1)] + (goal - next).lpNorm<1>();
        parent[next(0)][next(1)] = current;
        open.insert(std::make_pair(f[next(0)][next(1)], next));
      }
    }
  }
  path.clear();
  if (found) {
    current = goal;
    while (current != Eigen::Vector2i(-1, -1)) {
